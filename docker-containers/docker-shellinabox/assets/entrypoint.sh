...
certbot certonly --expand --authenticator standalone -n --agree-tos -m pknw1@hotmail.co.uk -d $DOMAIN && find /etc/letsencrypt -type f -iname*.pem -exec cp "{}" /etc/ssl/private/ \;
...
if [ "$SIAB_SSL" != "true" ]; then
	COMMAND+=" -t --cert=/etc/ssl/private/"
fi
